  ...
  config = mkMerge [
    (mkIf cfg.kubelet.enable {
      systemd.services.kubelet = {
        ...
        path = with pkgs; [ gitMinimal openssh utillinux iproute ethtool thin-provisioning-tools iptables ];
        preStart = ''
          ${pkgs.docker}/bin/docker load < ${infraContainer}
          rm /opt/cni/bin/* || true
        ...
        serviceConfig = {
          ExecStart = ''${cfg.package}/bin/kubelet \
            ...
            --pod-infra-container-image=gcr.io/google_containers/pause-amd64:latest \
            ${optionalString (cfg.kubelet.networkPlugin != null)
              "--network-plugin=${cfg.kubelet.networkPlugin}"} \
            ${optionalString (cfg.kubelet.volumePluginDir != null)
              "--volume-plugin-dir=${cfg.kubelet.volumePluginDir}"} \
